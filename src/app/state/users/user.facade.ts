...
import { catchError, defer, exhaustMap, map, mergeMap, Observable, of, switchMap, tap } from "rxjs";
...
export class UserFacade implements OnInitEffects {
    ...
    loginWithGoogle(preferredRole: UserRole): Observable<UserState> {
        this.store.dispatch(UserActions.googleLogin({ payload: { preferredRole } }))
        return this.user$;
    }
