...
export class DragListComponent {
  ...
  public dragDirs: QueryList<IgxDragDirective> =
    new QueryList<IgxDragDirective>();
  ...
  public onDragMove(event: IDragMoveEventArgs, itemIndex: number) {
    ...
    newIndex =
      newIndex < 0
        ? 0
        : newIndex >= this.employees.length
    ...
    if (
      (movingDown && newIndex > (this.newIndex ?? 0)) ||
      (!movingDown && newIndex < (this.newIndex ?? 0) && newIndex !== itemIndex)
    ) {
      ...
      if (currentLocation)
        elementToMove?.transitionTo(
          new IgxDragLocation(
            ...
            currentLocation?.pageY + prefix * this.listItemHeight
          ),
          { duration: this.animationDuration }
        );
    } else {
      ...
      const elementToMove = this.getDragDirectiveRef(
        this.employees[this.newIndex ?? 0].id
      );
