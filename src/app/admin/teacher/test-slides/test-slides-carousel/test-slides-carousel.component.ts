...
@Component({
  selector: 'app-test-slides-carousel',
  templateUrl: './test-slides-carousel.component.html',
  styleUrls: ['./test-slides-carousel.component.scss'],
})
export class TestSlidesCarouselComponent implements OnInit {
  ...
  addSlideMutation = this.slideService.addSlideForTest();
  updateSlideMutation = this.slideService.updateSlideForTest();
  chageSlideOrderMutation = this.slideService.changeSlideOrderForTest();

  public slides$: ITestSlide[] = [];
  public currentIndex = 0;
  public currentSlide?: ITestSlide;
  ...
  updateSlides(newSlides: ITestSlide[]) {
  ...
  private shiftElements(draggedIndex: number, targetIndex: number) {
    ...
    this.slides$.forEach((slide, index) => {
      this.chageSlideOrderMutation
        .mutate({ slideId: slide.id, testId: this.testId, order: index })
        .then();
    });
  }
}
